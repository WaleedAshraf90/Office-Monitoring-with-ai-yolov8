...
def speed_up_video(input_video_path, output_video_path, speed_factor=10):
    """
    تسريع الفيديو عن طريق تخطي بعض الإطارات.

    :param input_video_path: مسار الفيديو المدخل
    :param output_video_path: مسار الفيديو الناتج
    :param speed_factor: عامل السرعة (تخطي الإطارات). مثلاً، `2` يعني عرض كل إطار ثاني.
    """
    # فتح الفيديو
    cap = cv2.VideoCapture(input_video_path)
    
    # الحصول على معلومات الفيديو (مثل الـ FPS والحجم)
    fps = cap.get(cv2.CAP_PROP_FPS)  # الإطارات في الثانية
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    
    # إعدادات الفيديو الناتج
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Codec لـ mp4
    out = cv2.VideoWriter(output_video_path, fourcc, fps, (width, height))  # نفس معدل الإطارات
    ...
    while True:
        ...
        # إذا كانت الإطارات يجب تخطيها (حسب سرعة الفيديو المطلوبة)
        if frame_count % speed_factor == 0:
            out.write(frame)  # كتابة الإطار في الفيديو الناتج
...
def main(source_video):
    ...
    # Define the codec and create a VideoWriter object with increased FPS
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Codec لـ mp4
    out = cv2.VideoWriter('output_video1/work_desk_output.mp4', fourcc, 60.0, (int(cap.get(3)), int(cap.get(4))))  # زيادة معدل الإطارات هنا
